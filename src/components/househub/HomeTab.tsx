...
import { Member, Purchase, ActivityLog, RotationEntry, SUPPLIES, fmtDate, ago } from "@/lib/househub";
...
const HomeTab = ({ lastCleanMbr, lastCleanRec, purchases, actLog, getMember, thisCleanMbr, thisRotation, nextBuyer, isMyTurnClean, user, setTab }: HomeTabProps) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {/* Section label */}
      <p className="font-display font-bold italic text-xs text-ink-3 uppercase tracking-widest animate-fade-up" style={{ animationDelay: ".03s" }}>📋 House Status</p>
      ...
      >
        <div className="flex justify-between items-start mb-3.5">
          <div>
            <p className="text-xs font-bold text-ink-4 tracking-widest uppercase mb-2.5">🧹 Cleaning this Saturday</p>
          ...
          {thisRotation && (
            <div className="text-right">
              <p className="text-xs text-ink-4 font-bold mb-1.5">DATE</p>
              <p className="font-display font-bold text-sm text-forest">{fmtDate(thisRotation.date, { weekday: "short", day: "numeric", month: "short" })}</p>
            </div>
          )}
        </div>
        {isMyTurnClean && (
          <button className="w-full py-3.5 rounded-xl bg-forest font-bold text-cream shadow-[0_4px_20px_rgba(26,58,42,.3)] hover:bg-forest-2 active:scale-[0.96] transition-all" onClick={() => setTab("cleaning")}>Go to Cleaning →</button>
        )}
      </div>

      {/* Next buyer */}
      <div className="rounded-2xl border shadow-warm p-4 flex items-center gap-3.5 bg-gradient-to-br from-gold/10 to-gold/5 border-gold/20 transition-all hover:shadow-warm-md animate-fade-up" style={{ animationDelay: ".1s" }}>
      ...
      {/* Last cleaning */}
      {lastCleanRec && (
        <div className="rounded-2xl border shadow-warm p-4 flex items-center gap-3.5 bg-gradient-to-br from-sage/10 to-sage/5 border-sage/20 transition-all hover:shadow-warm-md animate-fade-up" style={{ animationDelay: ".13s" }}>
          ...
          <div className="flex-1">
            ...
            <p className="text-ink-3 text-xs mt-0.5">{fmtDate(lastCleanRec.cleaning_date, { weekday: "short", day: "numeric", month: "long" })}</p>
          </div>
          <span className="text-ink-4 text-xs">{ago(lastCleanRec.cleaning_date)}</span>
        </div>
      )}
            ...
            return (
              <div key={p.id} className="flex items-center gap-3.5 py-3 border-b border-border last:border-b-0 animate-fade-up" style={{ animationDelay: `${0.18 + i * 0.05}s` }}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-ink-3 text-xs mt-0.5">{fmtDate(p.purchase_date, { weekday: "short", day: "numeric", month: "long" })}</p>
                </div>
      ...
      {/* Activity log */}
      {actLog.length > 0 && (
        <div className="rounded-2xl border border-border bg-card shadow-warm p-5 animate-fade-up" style={{ animationDelay: ".22s" }}>
          <p className="text-xs font-bold text-ink-4 tracking-wider uppercase mb-3.5">📋 Activity Log</p>
          {actLog.slice(0, 5).map((l, i) => (
            <div key={l.id} className="flex items-center gap-3.5 py-3 border-b border-border last:border-b-0 animate-fade-up" style={{ animationDelay: `${0.24 + i * 0.04}s` }}>
              <span className="text-xl w-8 text-center">{l.icon || "📌"}</span>
              <div className="flex-1"><p className="font-semibold text-sm">{l.action}</p></div>
              <span className="text-ink-4 text-xs shrink-0">{ago(l.created_at)}</span>
            </div>
          ))}
        </div>
      )}
